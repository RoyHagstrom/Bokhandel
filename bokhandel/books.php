...
if (!empty($search_term)) {
    ...
    $conditions[] = "`Title` = ? OR `Author` = ? OR `Series` = ?";
    $bind_types .= "sss";
        ...
        <div id="search-form" class="flex justify-center w-full my-2 p-2">
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="get" class="flex items-center gap-2 max-w-xl">
                <div class="relative">
                    ...
                    <input type="search" <?php if(isset($_GET['search'])) { echo 'value="' . $_GET['search'] . '"'; } ?>  id="default-search" name="search" class="block w-full p-3 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search Mockups, Logos..." required />
                </div>
