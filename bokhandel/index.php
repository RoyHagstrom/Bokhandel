    ...
    <a href="books.php?search=<?= $series['SeriesName'] ?>" class="group w-full h-full">
        <div class="relative overflow-hidden rounded-lg shadow-md">
            ...
            <div class="absolute inset-0 flex flex-col justify-center items-center bg-black bg-opacity-70 group-hover:bg-opacity-75 transition-opacity duration-200 ease-in-out">
                <h2 class="text-base md:text-2xl font-semibold text-white mb-2 group-hover:opacity-100 transition-opacity duration-200 ease-in-out underline decoration-black"><?= $series['SeriesName'] ?></h2>
            </div>
        </div>
    </a>
...
<div class="container bg-white p-8 rounded-lg shadow-md w-full mx-auto mt-8">
    ...
    <div class="text-gray-700 dark:text-gray-300 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-6">
        <?php while ($user = $stmt->fetch_assoc()) { ?>
            <a class="block" href="my_books.php?uid=<?= $user['Username'] ?>">
                <div class="flex flex-col items-center border-b-2 border-gray-200 pb-4 mb-4">
                    <img src="<?= $user['Image'] ?>" alt="<?= $user['Username'] ?>" class="rounded-full h-16 w-16 sm:h-20 sm:w-20 md:h-24 md:w-24 lg:h-28 lg:w-28 object-cover mb-4" />
                    <div class="text-center">
                        <h2 class="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-black">#<?= $rank ?> <?= $user['Username'] ?></h2>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8 justify-center items-center">
        ...
        if ($featured_books_result->num_rows > 0) {
            while ($row = $featured_books_result->fetch_assoc()) {
                echo '<a href="singlebook.php?id=' . $row['BookID'] . '" class="block h-full bg-gray-100 dark:bg-gray-700 rounded-lg shadow-md overflow-hidden hover:bg-gray-200 dark:hover:bg-gray-800 transition duration-300 relative">';
...
<div class="bg-white container p-8 rounded-lg shadow-md w-full sm:w-130 mt-8">
    <h1 class="text-2xl font-semibold mb-6">Adult Books</h1>
    <div class="text-gray-700 dark:text-gray-300 mb-8">
        <p class="leading-relaxed text-sm sm:text-base lg:text-lg mb-4">
            Dive into captivating stories and thought-provoking narratives with our selection of adult books.
        </p>
    </div>
