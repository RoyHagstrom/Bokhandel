    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] !== UPLOAD_ERR_NO_FILE) {
        if ($_FILES["image"]["error"] === UPLOAD_ERR_OK) {
            ...
            if (!move_uploaded_file($tmp_name, $target)) {
                throw new RuntimeException("Error uploading image to $target");
            } else {
                $image = $newName;
        ...
        <form class="bg-white shadow-md rounded-lg p-8 mt-8" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?userid=<?php echo $userid; ?>" enctype="multipart/form-data">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div>
                    ...
                    <textarea id="bio" name="bio" class="border border-gray-400 p-2 w-full rounded-md" rows="8" maxlength="900"><?php echo $userInfo['Bio']; ?></textarea>
                </div>
                <div>
